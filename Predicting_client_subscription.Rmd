...
bank_data_mod <- bank_data %>% 
  ...
  select(-id_col) %>% 
  {.[sample(1:nrow(bank_data_mod)), ]} # randomize the data.
...
```{r}
# The grpreg package, see if it is better than gglasso below. 
...
```{r, eval= FALSE}
#The gglasso package. Does not seem to allow to set the threshold for predicting the 
#probabilities to the correct class (pi_0 in agresti book categorical analysis). 
#this means that a ROC curve is not going to be able to do. 
...
NN <- keras_model_sequential() %>% 
  layer_dense(units = 5, activation = "relu", input_shape = ncol(train_x)) %>% 
...
NN %>% fit(
  x = as.matrix(train_x),
  y = to_categorical(as.integer(unlist(train_y)) , num_classes = 2), 
  epochs = 20, 
  batch_size = 256, 
