...
abline(v = log(lambda_simplest), lty = 2, col = "lightgreen")
dev.off()
...
roc_curve_def_1se <- default_pred %>% 
  ...
  melt_roc(d = "y", m = c("glasso_def", "glasso_1se")) %>% 
  ggplot(aes(d = D, m = M, color = name)) + 
  geom_roc(labelround = 1, labelsize = 2.5, cutoffs.at = seq(1, 0.2, -0.1)) +
...
roc_curve_1se_simplest <- simplest_pred %>% 
  ...
  ggplot(aes(d = D, m = M, color = name)) + 
  geom_roc(labelround = 2, labelsize = 1.5) +
  geom_abline(slope = 1, intercept = 0) +
  labs(x = "1 - Specificity", y = "Sensitivity")
...
pruned_tree_1se <- prune(tree_model, cp = 4.133451e-03)
...
```{r}
# roc for train data
test3 <- predict(pruned_tree_1se, newdata = train_tree, type = "prob")
rownames(test3) <- NULL

test4 <- predict(model_glasso_simplest, newx = train_glasso_pred, type = "link") %>% 
  as.data.frame() %>% 
  rename("glasso" = "s0") %>% 
  mutate(y = train_under_samp$y, glasso = 1 / (1 + 2.718^-glasso))

roc_curve_tree_glasso <- test3 %>% 
  as.data.frame() %>% 
  select(-"0") %>% 
  rename("tree" = "1") %>% 
  ...
  ggplot(aes(d = D, m = M, color = name)) +
  geom_roc(labelround = 2, labelsize = 2.5, cutoffs.at = seq(1, 0.3, -0.1)) +
  geom_abline(slope = 1, intercept = 0) +
  labs(x = "1 - Specificity", y = "Sensitivity")

roc_curve_tree_glasso

ggsave("roc_curve_tree_glasso.png", width = 7.5, height = 7.5)
