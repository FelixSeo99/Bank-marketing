...
I DONT HAVE ACCESS TO THE ENTIRE DATASET SO I CANNOT SPLIT THE DATA IN TRAIN (4 YEARS) AND TEST (1 YEAR) AS TIME DEPENDENT. THIS IS BECAUSE MY DATA IS VERY UNBALENCE IN THE SENSE THAT ALL THE Y =1 IS IN THE TEST SET. NOTE THAT THE DATA IS ORDERED BY YEAR AND MONTH, WHICH WOULD HAVE BEEN CONVINIENT IF I DID NOT HAVE UNBALANCED DATA........
...
bank_data_mod <- bank_data %>% 
  dplyr::select(-duration, -default) %>% #probably include default?????
  mutate(
    temp1 = 1, 
    temp2 = 1,
    temp3 = 1, 
...
#################################
frobenius_norm <- c(
  sqrt(test_glasso_pred[, 1] %*% test_glasso_pred[, 1]),
  sqrt(test_glasso_pred[, 2] %*% test_glasso_pred[, 2]),
  sqrt(test_glasso_pred[, 3] %*% test_glasso_pred[, 3]),
  sqrt(test_glasso_pred[, 4] %*% test_glasso_pred[, 4]),
  sqrt(test_glasso_pred[, 5] %*% test_glasso_pred[, 5]),
  sqrt(test_glasso_pred[, 6] %*% test_glasso_pred[, 6]),
  sqrt(test_glasso_pred[, 7] %*% test_glasso_pred[, 7]),
  sqrt(test_glasso_pred[, 8] %*% test_glasso_pred[, 8]),
  sqrt(test_glasso_pred[, 9] %*% test_glasso_pred[, 9]),
  sqrt(test_glasso_pred[, 10] %*% test_glasso_pred[, 10]),
  sqrt(test_glasso_pred[, 11] %*% test_glasso_pred[, 11]),
  sqrt(test_glasso_pred[, 12] %*% test_glasso_pred[, 12]),
  sqrt(test_glasso_pred[, 13] %*% test_glasso_pred[, 13]),
  sqrt(sum(test_glasso_pred[, 14:24]^2)),
  sqrt(sum(test_glasso_pred[, 25:27]^2)),
  sqrt(sum(test_glasso_pred[, 28:37]^2)),
  sqrt(sum(test_glasso_pred[, 38:42]^2)),
  sqrt(sum(test_glasso_pred[, 43:45]^2))
)

cv_glasso_frobenius <- cv.gglasso(  # Frobenius norm for pf.
  train_glasso_pred,
  train_glasso_y,
  group = groups,
  loss = "logit",
...
model_glasso_frobenius <- gglasso(
  train_glasso_pred, #only predictors chosen
  train_glasso_y,
  group = groups,
  loss = "logit",
  lambda = seq(0.000005, 0.5, 0.001),
)  
 
#############################


cv_glasso_default <- cv.gglasso(  # defaults paramter for pf = sqrt(bs).
  train_glasso_pred,
  train_glasso_y,
  group = groups,
  loss = "logit",
...
# lambda within 1se from the model_glasso_1se
lambda_simplest <- which(cv_glasso_default$lambda == cv_glasso_default$lambda.1se) %>% 
  {cv_glasso_default$cvupper[.]} %>% 
  {which.max(cv_glasso_default$cvm < .)} %>% 
  {cv_glasso_default$lambda[.]}
...
test <- model_glasso %>% 
  ...
  as.data.frame() %>% 
  rename("pred" = "s0") %>% 
  mutate(y = test_under_samp$y, pred = 1 / (1 + 2.718^-pred))# %>% 
...
abline(v = log(cv_glasso_default$lambda.min), lty = 2)
abline(v = log(cv_glasso_default$lambda.1se), lty = 2)
dev.off()
...
model_glasso_simplest <- gglasso(
  train_glasso_pred, #only predictors chosen
  train_glasso_y,
  group = groups,
  loss = "logit",
  lambda = lambda_simplest
)

default_pred <- predict(model_glasso_default, newx = train_glasso_pred, type = "link") %>% 
  as.data.frame() %>% 
  rename("glasso_def" = "s0") %>% 
  mutate(y = train_under_samp$y, glasso_def = 1 / (1 + 2.718^-glasso_def))

test_pred <- predict(model_glasso_1se, newx = train_glasso_pred, type = "link") %>% 
  as.data.frame() %>% 
  rename("glasso_test" = "s0") %>% 
  mutate(glasso_test = 1 / (1 + 2.718^-glasso_test))

default_pred %>% 
  add_column("glasso_test" = test_pred$glasso_test) %>% 
  melt_roc(d = "y", m = c("glasso_def", "glasso_test")) %>% 
  ggplot(aes(d = D, m = M, color = name)) + 
  geom_roc(labelround = 1, n.cuts = 12) +
  geom_abline(slope = 1, intercept = 0) +
  labs(x = "1 - Specificity", y = "Sensitivity") 

ggsave("roc_curve_glasso.png")
...
pruned_tree_1se <- prune(tree_model, cp = 1.476233e-03)
...
rpart.plot(pruned_tree_1se, extra = 106, tweak = 1.1)
dev.off()
```

```{r, eval = FALSE}
# this package is said to be the original algorithm, but worse results here. cant find much on the 
# other package on how it calculates the tree so i will use it for now. 

test <- tree(y ~ ., data = train_tree, split = "gini", control = tree.control(nobs = length(train_tree$y), mindev = 0.001))
results <- prune.tree(test, method = "misclass")
test <- prune.tree(test, k = results$k[max(which(results$dev == min(results$dev)))], method = "misclass")
predict(test, newdata = test_tree) %>% 
  as.data.frame() %>% 
  rename("pred" = "1") %>% 
  mutate(pred = if_else(pred < 0.5, 0, 1)) %>% 
  select(pred) %>% 
  {sum(.$pred == test_tree$y) / length(test_tree$y)}
...
verification::roc.plot(test$y, hej, thresholds = seq(0, 1, 0.1), lwd = 7)
legend(x = "bottomright",          # Position
       legend = c("Tree", "Logistic"),  # Legend texts
...
```{r}
# roc for train data
test3 <- predict(tree_model, newdata = train_tree, type = "prob")
rownames(test3) <- NULL

test4 <- predict(model_glasso, newx = train_glasso_pred, type = "link") %>% 
  as.data.frame() %>% 
  rename("glasso" = "s0") %>% 
  mutate(y = train_under_samp$y, glasso = 1 / (1 + 2.718^-glasso))

reoc_curve_train <- test3 %>% 
  as.data.frame() %>% 
  select(-"0") %>% 
  rename("tree" = "1") %>% 
  mutate(glasso = test4$glasso, y = train_tree$y) %>%
  mutate(y = as.numeric(y), y = if_else(y == 2, 1, 0)) %>% 
  melt_roc(d = "y", m = c("glasso", "tree")) %>% 
  ggplot(aes(d = D, m = M, color = name)) +
  geom_roc(labelround = 2) +
  geom_abline(slope = 1, intercept = 0)

ggsave("roc_curve_train.png")
