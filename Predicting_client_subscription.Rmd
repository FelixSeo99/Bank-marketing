...
#abline(v = log(lambda_simplest), lty = 2)
dev.off()
...
roc_curve_tree_glasso <- test3 %>% 
  ...
  ggplot(aes(d = D, m = M, color = name)) +
  geom_roc(labelround = 2, labelsize = 2.5, cutoffs.at = 0.5)+ #seq(1, 0.3, -0.1)) +
  geom_abline(slope = 1, intercept = 0) +
  labs(x = "1 - Specificity", y = "Sensitivity")
...
```{r}
# confusion matrices for the tree and lasso
...
test_pred_tree %>% 
  ...
  select(- "0") %>%
  rename("pred" = "1") %>% 
  mutate(pred = if_else(pred >= 0.53, 1, 0), pred = as.factor(pred), ref = test_tree$y) %>% 
  {confusionMatrix(.$pred, .$ref)}

test_glasso_pred %>%  
  {predict(model_glasso_simplest, newx = ., type = "link")} %>% 
  as.data.frame() %>% 
  rename("pred" = "s0") %>% 
  mutate(
    ref = as.data.frame(test_glasso_y)$y, 
    pred = 1 / (1 + 2.718^-pred),
    pred = if_else(pred >= 0.5, 1, -1),
    pred = as.factor(pred),
    ref = as.factor(ref)
  ) %>% 
  {confusionMatrix(.$pred, .$ref)}
