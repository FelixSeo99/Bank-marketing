...
tree_parameters <- rpart.control(minsplit = 60, maxcompete = 10, maxdepth = 20, cp =  0.064363744)
tree_model <- rpart(y ~ ., data = train_tree, method = "class", control = tree_parameters)
rpart.plot(tree_model, extra = 106, cex = 0.5)
...
test2 %>% 
  ...
  rename("yes" = "1") %>% 
  select(yes) %>% 
  {verification::roc.plot(test_tree$y, .$yes, thresholds = seq(0, 1, 0.2))}
...
hej2 <- test2 %>%
  as.data.frame() %>% 
  add_column(test_tree$y) %>% 
  rename("pred" = "1", "remove" = "0", "true" = "test_tree$y") %>% 
  select(-remove)
  

ggplot() +
  geom_roc(data = test, aes(m = pred, d = y), n.cuts = 20, labelround = 2) +
  geom_roc(data = hej2, aes(m = pred, d = true), n.cuts = 20) +
