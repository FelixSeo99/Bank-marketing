...
test_pred <- predict(model_glasso_test, newx = train_glasso_pred, type = "link") %>% 
  as.data.frame() %>% 
...
default_pred %>% 
  ...
  geom_roc(labelround = 2, n.cuts = 20) +
...
tree_parameters <- rpart.control(minsplit = 50, maxcompete = 4, maxdepth = 30, cp =  1.771479e-03)
