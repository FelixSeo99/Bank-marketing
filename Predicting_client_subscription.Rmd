...
test_pred <- predict(
  ...
  type = "link"  #link gives pred values. convert to prob by 1/(1+e^-x)
)
...
tree_model <- rpart(y ~ ., data = bank_data_mod, method = "class")

predict(tree_model, data = bank_data_mod, type = "class") %>% 
  as.vector() %>% 
  as.factor() %>% 
  confusionMatrix(as.factor(bank_data_mod$y))
