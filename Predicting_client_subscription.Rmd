...
test_pred <- predict(
  model_glasso, 
  newx = test_glasso_pred, 
  type = "class",  #link gives pred values. convert to prob by 1/(1+e^-x)
)

test_under_samp %>% 
  select(y) %>% 
  mutate(y = if_else(y == 0, -1, y), y = as.factor(y)) %>% 
  {.$y} %>%  
...
tree_parameters <- rpart.control(minsplit = 60, maxcompete = 10, maxdepth = 5, cp = 0.003)
tree_model <- rpart(y ~ ., data = train_tree, method = "class", control = tree_parameters)
rpart.plot(tree_model, extra = 106)


predict(tree_model, test_tree, type = "class") %>% 
  as.vector() %>% 
  as.factor() %>%   
  confusionMatrix(as.factor(test_tree$y))
